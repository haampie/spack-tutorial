...
then explain how to expand, configure, and build software in them.
...
If you *only* want to check which environment you are in, you can use
`spack env status`:
...
As a shorthand, you can use the `install --add` flag to accomplish
...
Let's run `spack concretize --force` (or `-f` for short) to make
...
Since these files are key to reproducing builds, let's start with them.
...
not only do we get the same root specs, but all of the packages are
